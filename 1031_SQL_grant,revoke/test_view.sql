-- view를 생성하려면 관리자 권한을 가진 계정의 view 생성 권한을 부여
-- 생성 권한을 부여해야한다.
-- 관리자 계정이 권한 부여
...
/* 복합 view: 테이블 여러개를 사용하여 생성한 view
            ...
            - insert, update, delete가 수행되지 않는다.
            - 여러개의 select를 모으고 단순 select를 사용하여 조회할 때. */
...
-- 복합 view는 DML(insert, update, delete)이 사용되지 않는다.
insert into car_view(maker, model, car_year, price, cc, car_option)
values ('현대', '벨로스터', 2017, 3000, 1999, '문짝 3개,ABS');
