...
declare
	...
	-- 학생들의 정보를 저장할 table 선언
		type stu_table is table of stu_record
			index by binary_integer;
		
	-- table 사용한 학생들의 정보를 처리할 변수 설정
...
begin 
	--table 변수에 값 할당
	class4_tab(1).num:= 1;
	class4_tab(1).name :='제스퍼'; 
	class4_tab(1).oracle_score := 98;
	
	class4_tab(2).num:= 2;
	class4_tab(2).name := '준걸찡'; 
	class4_tab(2).oracle_score := 85;
	
	class4_tab(3).num:= 3;
	class4_tab(3).name := '임의신'; 
	...
	dbms_output.put_line('응시인원: '||class4_tab.count);
