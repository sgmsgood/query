...
begin
    -- 3. 값 넣기: 인덱스를 사용해서 값을 넣음
    dbms_output.put_line ('table 변수 방의 개수 '||age_arr.count); --> 값 추가 전 방의 개수: 0
	age_arr(1):= 20;
    age_arr(2):= 25;
	age_arr(3):= 27;
	age_arr(4):= 26;

	dbms_output.put_line ('table 변수 방의 개수 '||age_arr.count); --> 값 추가 후 방의 개수: 4
